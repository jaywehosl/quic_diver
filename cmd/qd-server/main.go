...
func main() {
	...
	cfg := server.Config{
		...
		ExitsPath:         "/qd-exits",
		SubscriptionPath:  server.SubscriptionPath,
	...
	if store != nil {
		...
		// Метрики соседей для балансировки auto:<тег>. Снимаются с уже поднятых
		// связей — QUIC меряет RTT сам, отдельный пинг заводить незачем.
		go links.PollStats(ctx)
