...
type Config struct {
	...
	// Autoconnect — заворачивать трафик сразу при старте сервиса. false — сервис
	// живёт и отдаёт панель, а перехват включается только по команде
	// пользователя (туннель и перехват разделены намеренно).
	//
	// По умолчанию true: пока панели нет, выключенный автостарт означал бы
	// сервис, который поднялся и ничего не делает.
...
type Panel struct {
	...
	// доступна из сети.
	Addr string `json:"addr"`
	// Open — открывать панель в браузере при старте.
	//
	// Для сервиса в фоне это выключают: браузер, всплывающий при каждом входе
	// в систему, раздражает сильнее, чем помогает.
	Open bool `json:"open"`
}
