...
func TestOutboundsCRUD(t *testing.T) {
	s := openTemp(t)
	ctx := context.Background()

	// direct + chain
	if err := s.PutOutbound(ctx, OutboundRow{Label: "direct", Type: OutDirect, Enabled: true}); err != nil {
		t.Fatal(err)
	}
	if err := s.PutOutbound(ctx, OutboundRow{
		Label: "eu", Type: OutChain, Addr: "1.2.3.4:443", Authority: "eu.example", Token: "qd_node", Enabled: true,
	}); err != nil {
		t.Fatal(err)
	}
	list, err := s.ListOutbounds(ctx)
	if err != nil || len(list) != 2 {
		t.Fatalf("список: %v (%d)", err, len(list))
	}
	// порядок по label: direct, eu
	if list[0].Label != "direct" || list[1].Label != "eu" {
		t.Fatalf("порядок: %v", list)
	}
	if list[1].Addr != "1.2.3.4:443" || list[1].Token != "qd_node" {
		t.Fatalf("chain-поля: %+v", list[1])
	}

	// upsert (смена addr) не плодит дублей
	if err := s.PutOutbound(ctx, OutboundRow{Label: "eu", Type: OutChain, Addr: "9.9.9.9:443", Enabled: true}); err != nil {
		t.Fatal(err)
	}
	list, _ = s.ListOutbounds(ctx)
	if len(list) != 2 || list[1].Addr != "9.9.9.9:443" {
		t.Fatalf("upsert: %+v", list)
	}

	// выключенный не попадает в список
	if err := s.PutOutbound(ctx, OutboundRow{Label: "eu", Type: OutChain, Addr: "9.9.9.9:443", Enabled: false}); err != nil {
		t.Fatal(err)
	}
	if list, _ = s.ListOutbounds(ctx); len(list) != 1 {
		t.Fatalf("выключенный виден: %d", len(list))
	}

	// chain без addr — отказ
	if err := s.PutOutbound(ctx, OutboundRow{Label: "bad", Type: OutChain, Enabled: true}); err == nil {
		t.Fatal("chain без addr принят")
	}
	// удаление
	if err := s.DeleteOutbound(ctx, "direct"); err != nil {
		t.Fatal(err)
	}
	if list, _ = s.ListOutbounds(ctx); len(list) != 0 {
		t.Fatalf("после удаления: %d", len(list))
	}
}
